...
*npersist::UInt          # Number of persisting directions (must be <population size) TODO implement this
...
*optimizer::AbstractOptimizer # Can be either: :GradDesc, :Momentum, TODO: Adam
*printiter::Int          # How often to print
"""
mutable struct EsHyperparam
    std::Number             # Standard deviation of the random exploration
    population_size::UInt   # Population size
    df_clip::Number         # Maximum df allowed in one perturbed direction, clamps bigger abs. values
    maxiter::UInt           # Maximal allowed number of iterations
    ...
    function EsHyperparam(std, ps, df_clip, maxiter, maxevals, npersist, dx_tol, f_tol, optimizer, printiter)
        @assert std>0
        @assert df_clip>0.0
        @assert maxiter>0
        @assert maxevals>0
...
InitEsHyperparam(x) = EsHyperparam(0.1, length(x), Inf, 1000, Inf, ceil(UInt, length(x)/10), 1e-6, -Inf, GradDesc(0.01), 1)

function show(io::IO, par::EsHyperparam)
    ...
    println("\toptimizer:\t\t $(par.optimizer)")
