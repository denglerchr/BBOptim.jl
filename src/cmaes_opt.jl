function optimize(f::Function, x0::Vector, hyp_par::CMAES; prefunc::Function = x->nothing, workerpool::AbstractWorkerPool = default_worker_pool())
    ...
    weights = log(hyp_par.population_size/2+0.5) .- log.(1:(hyp_par.population_size/2)) #array for weighted recombination
    normalize!(weights, 1) #normalize weights
    mueff = 1.0/sum(x->x^2, weights)

    #Strategy parameter setting: Adaptation
    cc = (4 + mueff/N) / (N + 4 + 2*mueff/N) #time constant for cumulation of the covariance matrix C
    cs = (mueff + 2) / (N + mueff + 5) #time constant for cumulation for variance control
    c1 = 2 / ((N + 1.3)^2 + mueff) #learning rate for rank-one update of C
    cmu = min(1 - c1, 2 * (mueff- 2 + 1/mueff) / ((N + 2)^2 + mueff)) # and for rank-mu update
    damps = 1 + 2*max(0, sqrt((mueff - 1 ) / (N + 1)) - 1) + cs #damping for sigma

    #Initialize dynamic (internal) strategy parameters and constants
    pc = zeros(N) #evolution paths for C and sigma
    ps = zeros(N)

    B = I #defines the coordinate system
    D = ones(N) #defines the scaling
    C_full = Matrix{Float64}(I, N, N) #covariance matrix C
    C = Symmetric(C_full)
    invsqrtC = I #C^-0.5
    ...
    #Generation Loop
    counteval = 0
    arx = zeros(N, hyp_par.population_size)
    artemp = zeros(N, mu+1)
    ...
    for iter = 1:hyp_par.maxiter     #counteval < stopeval
        ...
        arx .= xmean .+ hyp_par.std*B*(D .* randn(N, hyp_par.population_size))
        ...
        cscale = (1 - c1 - cmu + c1*(1 - hsig)*cc*(2 - cc))
        BLAS.syrk!('U', 'N', 1.0, artemp, cscale, C_full)
